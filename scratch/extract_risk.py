...
for i, line in enumerate(lines):
    if "if (page === 'risk') {" in line and i > 650:
        start_idx = i
        break
...
export default function RiskTab({{ 
    t, setPage, riskLoading, riskRoster, riskFilter, setRiskFilter 
}}) {{
{block.replace("if (page === 'risk') {", "").rstrip()[:-1]}
}}
"""
        os.makedirs(r'c:\Users\ayush\Desktop\final year draft1\src\components\dashboard\admin\tabs', exist_ok=True)
        with open(r'c:\Users\ayush\Desktop\final year draft1\src\components\dashboard\admin\tabs\RiskTab.jsx', 'w', encoding='utf-8') as f:
            f.write(component_content)
        print("RiskTab.jsx created.")

        # Now remove it from AdminDashboard and replace with component
        replacement = f"    if (page === 'risk') {{\n        return <RiskTab \n            t={{t}} setPage={{setPage}}\n            riskLoading={{riskLoading}} riskRoster={{riskRoster}} riskFilter={{riskFilter}} setRiskFilter={{setRiskFilter}}\n        />;\n    }}\n"
